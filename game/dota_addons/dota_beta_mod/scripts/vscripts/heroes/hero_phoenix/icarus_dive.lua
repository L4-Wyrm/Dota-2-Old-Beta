...
function CheckToInterrupt( event )
	...
	if caster:IsStunned() or caster:IsHexed() or caster:IsFrozen() or caster:IsNightmared() or caster:IsOutOfGame() or caster:IsRooted() or caster:SpendMana(mana_cost, ability) then
...
function CheckToDisarm( event )
	local caster		= event.caster
	local target		= event.target
	local ability		= event.ability
	local modifierName	= event.modifier_disarm_name
